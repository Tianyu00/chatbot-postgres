...
db.execute("CREATE TABLE IF NOT EXISTS words(id INT PRIMARY KEY, word TEXT UNIQUE)")
db.execute("CREATE TABLE IF NOT EXISTS sentences(id INT PRIMARY KEY, sentence TEXT UNIQUE, used INT NOT NULL DEFAULT 0)")
db.execute("CREATE TABLE IF NOT EXISTS associations (id INT PRIMARY KEY, word_id INT NOT NULL, sentence_id INT NOT NULL, weight REAL NOT NULL)")
...
def response(B,H):
    ...
    for ri in r:
        B = ri
        break
    db.execute("UPDATE sentences SET used=used+1 WHERE id='"+str(B[0])+"'")
    print(ri)
    return B[1]
